...
const FallbackData = {
    ...
    products: [{ id: 1000, name: "DJI Neo 2", cost: "1.660,00", sell: "2.324,00", line: "Consumer" }],
...
const Utils = {
    ...
    reload: () => { localStorage.clear(); location.reload(); },
...
class Store {
    ...
    async loadData() {
        try {
            console.log("🛰️ Ativando Realtime - Galvão Drones");
    ...
    _normalizeMarketing(assets) {
        return assets.map(a => (!a.variations && a.downloadUrl) ? { ...a, variations: [{ name: 'Padrão', url: a.downloadUrl }] } : a);
    }
}
...
class AuthService {
    static async login(username, password) {
        if (appStore.state.isLocked) return { success: false, msg: "Bloqueado." };
        ...
        return { success: false, msg: "Incorreto." };
    }
}
 ...
 */
const Templates = {
    login: () => `<div class="min-h-screen flex items-center justify-center p-4 bg-gray-100 dark:bg-black"><div class="bg-white dark:bg-zinc-900 w-full max-w-md p-8 rounded-[2.5rem] shadow-2xl border dark:border-zinc-800"><div class="flex justify-center mb-8"><div id="login-logo-trigger" class="w-24 h-24 bg-gray-50 dark:bg-zinc-800 rounded-full flex items-center justify-center"><img src="https://imgur.com/ny5U9KV.png" class="w-16 h-16"></div></div><h2 class="text-center text-2xl font-black text-brand-green dark:text-white uppercase italic mb-8">Galvão Drones</h2><form onsubmit="App.handleLogin(event)" class="space-y-4"><input type="text" id="username-input" placeholder="Usuário" class="w-full px-6 py-4 rounded-2xl bg-gray-50 dark:bg-zinc-800 border-none font-bold outline-none uppercase"><input type="password" id="password-input" placeholder="Senha" class="w-full px-6 py-4 rounded-2xl bg-gray-50 dark:bg-zinc-800 border-none font-bold outline-none"><div id="error-message" class="hidden p-3 bg-red-50 text-red-500 text-xs font-bold text-center rounded-xl"></div><button type="submit" class="w-full py-4 bg-brand-orange text-white font-black rounded-2xl shadow-lg uppercase text-xs">Entrar no Sistema</button><button type="button" onclick="App.handleVisitorLogin()" class="w-full mt-2 text-[10px] font-bold text-gray-400 uppercase">Modo Visitante</button></form></div></div>`,
    getModals: () => `<div id="sidebar-overlay" onclick="document.getElementById('sidebar-menu').classList.add('-translate-x-full'); this.style.display='none'"></div><div id="admin-login-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4"><div class="bg-white dark:bg-zinc-900 w-full max-w-sm p-6 rounded-[2rem] text-center shadow-2xl"><h3 class="text-xl font-black text-brand-orange uppercase italic mb-6">Acesso Administrativo</h3><div id="keypad-display" class="h-12 bg-gray-100 dark:bg-zinc-800 rounded-xl mb-4 flex items-center justify-center text-2xl font-black dark:text-white"></div><div id="virtual-keypad" class="keypad-grid mb-4"></div><button onclick="Modals.close('admin-login-modal')" class="text-xs font-bold text-gray-400 uppercase">Cancelar</button></div></div><div id="admin-form-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div class="bg-white dark:bg-zinc-900 w-full max-w-md p-6 rounded-[2rem] shadow-2xl"><h3 id="admin-modal-title" class="font-black text-brand-orange uppercase italic mb-4"></h3><div id="admin-dynamic-form" class="space-y-3 mb-6"></div><div class="flex gap-2"><button onclick="Modals.close('admin-form-modal')" class="flex-1 py-3 bg-gray-100 dark:bg-zinc-800 font-bold rounded-xl text-xs">Cancelar</button><button onclick="AdminController.save()" class="flex-1 py-3 bg-green-600 text-white font-black rounded-xl text-xs">Salvar</button></div></div></div><div id="delete-confirm-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4"><div class="bg-white dark:bg-zinc-900 w-full max-w-xs p-6 rounded-[2rem] text-center shadow-2xl"><h3 class="font-black dark:text-white uppercase mb-6">Confirmar exclusão?</h3><div class="flex gap-2"><button onclick="Modals.close('delete-confirm-modal')" class="flex-1 py-3 bg-gray-100 dark:bg-zinc-800 font-bold rounded-xl text-xs">Voltar</button><button onclick="AdminController.confirmDelete()" class="flex-1 py-3 bg-red-500 text-white font-black rounded-xl text-xs">Excluir</button></div></div></div>`
};

const ViewRenderer = {
    dashboardLayout: () => `<div id="dashboard-view" class="flex h-screen overflow-hidden bg-gray-50 dark:bg-black">${ViewRenderer._sidebar()}<div class="flex-1 flex flex-col min-w-0 overflow-y-auto font-sans relative">${ViewRenderer._header()}<main id="main-content" class="p-4 sm:p-8 space-y-6 fade-in"></main></div></div>`,
    _sidebar: () => {
        const u = appStore.state.currentUser;
        return `<aside id="sidebar-menu" class="fixed inset-y-0 left-0 w-72 bg-white dark:bg-zinc-900 shadow-2xl transform -translate-x-full lg:relative lg:translate-x-0 transition-transform duration-300 z-40 flex flex-col border-r dark:border-zinc-800"><div class="p-8 border-b dark:border-zinc-800 flex flex-col items-center"><div class="w-24 h-24 rounded-full bg-brand-orange text-white flex items-center justify-center font-bold text-3xl mb-4 profile-avatar-div" style="background-image: url('${u.avatar || ''}')">${u.avatar ? '' : u.name[0]}</div><p class="font-black text-sm dark:text-white uppercase italic">${u.name}</p><p class="text-[10px] text-brand-green font-bold uppercase mt-1">${u.role}</p></div><nav class="flex-1 px-4 py-6 space-y-2"><button onclick="App.navigate('dashboard')" class="w-full px-6 py-4 rounded-2xl font-bold text-left uppercase text-sm italic">Dashboard</button><button onclick="App.navigate('consultas')" class="w-full px-6 py-4 rounded-2xl font-bold text-left uppercase text-sm italic">Novos</button><button onclick="App.navigate('usados')" class="w-full px-6 py-4 rounded-2xl font-bold text-left uppercase text-sm italic">Seminovos</button><button onclick="App.navigate('admin')" class="w-full px-6 py-4 rounded-2xl font-bold text-left text-red-500 uppercase text-sm italic">Gestão</button></nav></aside>`;
    },
...
const App = {
    init: async () => { await appStore.loadData(); App.renderLogin(); },
    ...
    handleLogin: async (e) => { e.preventDefault(); const res = await AuthService.login(document.getElementById('username-input').value, document.getElementById('password-input').value); if (res.success) App.renderApp(); },
